...
import { HtChevronLeftOutline } from "../icons/index.js";
...
export const Calendar = (props: CalendarProps) => {
  ...
  const [yearRangeStart, setYearRangeStart] = React.useState(() => new Date().getFullYear() - 5);
  ...
  const getCalendarDays = React.useCallback((monthDate: Date) => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) days.push(undefined);
    for (let i = 1; i <= daysInMonth; i++) days.push(new Date(year, month, i));
    return days;
  }, []);

  const months = React.useMemo(() => Array.from({ length: numberOfMonths }, (_, i) => addMonths(currentMonth, i)), [currentMonth, numberOfMonths]);

  const isPrevDisabled = React.useMemo(
    () => (minDate ? isBefore(subMonths(currentMonth, 1), startOfMonth(minDate)) : false),
    [currentMonth, minDate],
  );
  ...
  const years = React.useMemo(() => Array.from({ length: 12 }, (_, i) => yearRangeStart + i), [yearRangeStart]);
  const isPrevYearRangeDisabled = minDate ? yearRangeStart <= minDate.getFullYear() : false;
  const isNextYearRangeDisabled = maxDate ? yearRangeStart + 11 >= maxDate.getFullYear() : false;

  const openYearView = React.useCallback(() => {
          ...
          <div className="flex w-full items-center justify-between gap-x-4">
            <button
              ...
              className={cn(
                ...
                isPrevYearRangeDisabled && "cursor-not-allowed opacity-50",
              )}
            ...
            <button
              ...
              className={cn(
                "grid size-7 place-items-center rounded border border-neutral-400 transition-opacity",
                isNextYearRangeDisabled && "cursor-not-allowed opacity-50",
              )}
              onClick={() => setYearRangeStart((p) => p + 12)}
            >
              <HtChevronLeftOutline className="size-4 rotate-180" />
            </button>
          </div>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    !isCurrent && !isDisabled && "hover:bg-neutral-200",
              ...
              <div key={monthDate.toISOString()} className="w-full max-w-72 space-y-2">
                <div className="flex w-full items-center justify-between gap-x-4">
                  ...
                  <div className="flex-1 text-center text-sm font-medium">
                    ...
                    <button type="button" onClick={openYearView} className="underline-offset-2 hover:underline" aria-label="Pick a year">
                ...
                <div className="grid w-full grid-cols-7 gap-x-1">
                  {DAYS_OF_WEEK.map((day) => (
                    <div key={day} className="flex aspect-square items-center justify-center text-sm font-medium text-neutral-500">
                      {day}
                    </div>
                  ))}
                </div>
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          rangeStart && "to-primary-100 rounded-l-lg bg-linear-to-r from-transparent",
                          rangeEnd && "to-primary-100 rounded-r-lg bg-linear-to-l from-transparent",
