...
export const MultiSelect = ({
    ...
    (optionValue: string) => {
      ...
      if (isSelected) {
        ...
        if (maxSelectable && value.length >= maxSelectable) {
          toast.info(`Maximum ${maxSelectable} selection${maxSelectable !== 1 ? "s" : ""} allowed.`);
          return;
        }
  ...
  return (
    <div className={cn("w-full space-y-0.5", wrapperClassName)}>
      {label && (
        <p className={cn("text-sm font-medium text-gray-700", labelClassName, required && "after:ml-1 after:text-red-500 after:content-['*']")}>
          {label}
        </p>
      )}
      <Popover>
        <PopoverTrigger asChild disabled={disabled}>
          ...
          >
            <span className={value.length === 0 ? "text-muted-foreground" : undefined}>
              {selectedLabels.length > 0 ? selectedLabels.join(", ") : (placeholder ?? "Select values...")}
            </span>
            <HtChevronDownOutline className="shrink-0 text-gray-400" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="p-0" style={{ width: "var(--radix-popper-anchor-width)" }}>
          <Command loop className="min-w-fit">
            ...
            <CommandList>
              <CommandEmpty className="text-center text-xs text-gray-400">No results found.</CommandEmpty>
              <CommandGroup>
                {options.map((option) => {
