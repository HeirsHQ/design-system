...
import { DIAL_CODES } from "../constants/form.js";
...
export const PhoneInput = ({ className, label, onChange, value, wrapperClassName, error, disabled }: Props) => {
  const isControlled = value !== undefined;

  const [internalDialCode, setInternalDialCode] = useState(DIAL_CODES[0]?.code ?? "");
  const [internalNumber, setInternalNumber] = useState("");

  const { dialCode, phoneNumber } = isControlled ? parse(value) : { dialCode: internalDialCode, phoneNumber: internalNumber };
  ...
  return (
    <div className={cn("space-y-1", wrapperClassName)}>
      ...
      >
        <Select onValueChange={handleDialChange} value={dialCode} disabled={Boolean(disabled)}>
      ...
      {isInvalid && <p className="text-xs text-red-500">Phone number must be between 7 and 15 digits.</p>}
