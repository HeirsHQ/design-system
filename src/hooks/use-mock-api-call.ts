...
interface UseMockApiCallProps<T> {
...
interface UseMockApiCallReturn<T> {
 ...
 * }
 *
 * const { isLoading, response, triggerCall } = useMockApiCall<User>({
 ...
 * ```
 */
export const useMockApiCall = <T>({
  loadTime = 1000,
  expectedResponse,
  mockData,
  errorMessage = "An error occurred",
}: UseMockApiCallProps<T>): UseMockApiCallReturn<T> => {
